		...
		$.each(data.items, function (i, f) { 
			if( f.display =='true') {
				...
				if(f.html_content){
					...
					if(f.a_href_link){
						add_content += "<a href='"+f.a_href_link+"' class='banner_link' target='_blank'><img title='"+f.img_title+"' src='"+$jsonFilePath+"/images/" + f.banner_img + "' / ></a>";
					}else{
						add_content += "<img title='"+f.img_title+"' src='"+$jsonFilePath+"/images/" + f.banner_img + "' / >";
					}
				}	
				add_content += '</div></div>';
